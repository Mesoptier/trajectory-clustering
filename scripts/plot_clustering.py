...
def plot_clustering(filename, figure_name, radius, dpi=1200):
    file = open(filename, "r")
    colors = ['m', 'g', 'c', 'r', 'y', 'b']
    center_colors = ['m', 'g', 'c', 'r', 'y', 'b']
    ax = plt.gca()
    k = int(file.readline())
    clustering = []
    for i in range(k):
    ...
    for i in range(len(clustering)):
        ...
        for curve in cluster["curves"]:
            for j in range(0, len(curve)-2, 2):
                plt.plot((curve[j], curve[j+2]), (curve[j+1], curve[j+3]), colors[i], linewidth=0.1)
