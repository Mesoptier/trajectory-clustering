...
namespace {
    Point lowest_with_x(Points const& matching, distance_t x, std::size_t i) {
        ...
        distance_t t = (x - matching[i - 1].x)
            / (matching[i].x - matching[i - 1].x);
    ...
    Point lowest_with_y(Points const& matching, distance_t y, std::size_t i) {
        ...
        distance_t t = (y - matching[i - 1].y)
            / (matching[i].y - matching[i - 1].y);
        return matching[i - 1] * (1 - t) + matching[i] * t;
    }
}
...
MatchingBand::MatchingBand(Curve const& curve_x, Curve const& curve_y,
    ...
    PointID max_x = 0;
    while (max_x + 1 < curve_x.size() && curve_x.curve_length(
            static_cast<PointID::IDType>(p.x), max_x + 1) <= radius) {
        ++max_x;
    }
    ...
    PointID max_y = 0;
    while (max_y + 1 < curve_y.size() && curve_y.curve_length(
            static_cast<PointID::IDType>(p.y), max_y + 1) <= radius) {
        ++max_y;
    }
    ...
    while (!approx_equal(p, matching.back())) {
        ...
        if (max_x + 1 < curve_x.size()) {
            p = std::min(p, lowest_with_x(matching,
                curve_x.curve_length(max_x + 1) - radius, i), compare);
        }
        if (max_y + 1 < curve_y.size()) {
            p = std::min(p, lowest_with_y(matching,
                curve_y.curve_length(max_y + 1) - radius, i), compare);
        }
        ...
        while (curve_y.curve_length(min_y_excl) <= p.y - radius + ABS_TOL)
            ++min_y_excl;
        while (max_x + 1 < curve_x.size()
                && curve_x.curve_length(max_x + 1) <= p.x + radius + ABS_TOL)
            ++max_x;
        while (max_y + 1 < curve_y.size()
                && curve_y.curve_length(max_y + 1) <= p.y + radius + ABS_TOL)
            ++max_y;
        ...
        for (PointID ix = min_x_incl; ix <= max_x; ++ix) {
            ...
            if (!lower_y_at_x.at(ix).getPoint().valid()
                    || lo < lower_y_at_x.at(ix))
                lower_y_at_x.at(ix) = lo;

            if (!upper_y_at_x.at(ix).getPoint().valid()
                    || hi > upper_y_at_x.at(ix))
        ...
        for (PointID iy = min_y_incl; iy <= max_y && iy < curve_y.size(); ++iy) {
            ...
            if (!lower_x_at_y.at(iy).getPoint().valid()
                    || lo < lower_x_at_y.at(iy))
                lower_x_at_y.at(iy) = lo;

            if (!upper_x_at_y.at(iy).getPoint().valid()
                    || hi > upper_x_at_y.at(iy))
