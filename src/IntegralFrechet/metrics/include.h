...
distance_t integrate_linear_cost_l1(Cell const& cell, Point const& s,
    Point const& t);
 ...
 */
template<ParamMetric param_metric, ImageMetric im_metric>
distance_t integrate_linear(Cell const& cell, Point const& s, Point const& t) {
    if (im_metric == ImageMetric::L2)
        return integrate_linear_cost(cell, s, t)
            * integrate_linear_dist<param_metric>(cell, s, t);
    else if (im_metric == ImageMetric::L1)
        return integrate_linear_cost_l1(cell, s, t)
            * integrate_linear_dist<param_metric>(cell, s, t);
}
...
distance_t compute_cost(Cell const& cell, Points const& matching) {
    ...
    for (std::size_t i = 1; i < matching.size(); ++i) {
        cost += integrate_linear<param_metric, im_metric>(cell, matching[i - 1], matching[i]);
    }
    return cost;
}
#endif
