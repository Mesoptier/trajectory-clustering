...
Point& Point::operator/=(distance_t div) {
    x /= div;
    y /= div;
    return *this;
}

Point Point::operator/(distance_t div) const {
    return {x / div, y / div};
}
...
distance_t norm(Point const& point, Norm p) {
    switch (p) {
        ...
        case Norm::L2:
            return std::sqrt(pow2(point.x) + pow2(point.y));
        case Norm::LInf:
...
Interval IntersectionAlgorithm::intersection_interval(Point circle_center,
    ...
    // Here we need the guarantee that if the free interval has length at least
    // eps then at mid the distance is <= radius.
    // This is an assumption about the precision of distance_t computations
    // All remaining rules are free of such assumptions!
    // (except for trivial ones like this: x + y and x - y have distance at most
    // 2y up to negligible error)
    if (!smallDistAtMid) {
        if (outer != nullptr)
            *outer = Interval();
    ...
    if (smallDistAtZero) {
        ...
        if (innershift >= outershift &&
            ...
            distance_t left = 0., right = std::min(mid, 1.);
            ...
            // If smallDistanceAt(0.) then begin would already be set
            // (fourth rule).
            // If !smallDistanceAt(right), then either !smallDistanceAt(mid),
            // contradicting the very first rule, or mid >= 1. and
            // smallDistanceAt(1.), contradicting the third rule.
            // Finally, since !smallDistanceAt(left) we cannot have mid <= 0 by
            // the second rule. Thus, right = min(mid,1) >= 0. = left
...
namespace {
    ...
    Circle calcCircle(Point p1, Point p2, Point p3)
    {
        ...
        // Avoid division by zero. Due to non-colinearity n1.x is non-zero after
        // the swap.
        if (n1.x == 0) {
            std::swap(n1, n2);
            std::swap(q1, q2);
        ...
        // if the points p1, p2, p3 are collinear
        if (std::abs(b) < epsilon) {
            // rather naive
            if (inCircle(p1, circleFromTwo(p2,p3)))
                return circleFromTwo(p2,p3);
            if (inCircle(p2, circleFromTwo(p1,p3)))
                return circleFromTwo(p1,p3);
...
Circle calcMinEnclosingCircle(Points points) {
    ...
    for (PointID i = 2; i < points.size(); ++i) {
        auto p_i = points[i];
        if (inCircle(p_i, current_circle))
            continue;
        ...
        for (PointID j = 1; j < i; ++j) {
            auto p_j = points[j];
            if (inCircle(p_j, current_circle))
                continue;
            ...
            for (PointID k = 0; k < j; ++k) {
                auto p_k = points[k];
                if (inCircle(p_k, current_circle))
                    continue;
