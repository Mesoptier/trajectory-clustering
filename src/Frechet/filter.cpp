...
bool Filter::isPointTooFarFromCurve(Point fixed, Curve const& curve,
    ...
    if (fixed.dist_sqr(curve.front()) <= dist_sqr
            || fixed.dist_sqr(curve.back()) <= dist_sqr)
        return false;
    ...
    for (PointID pt = 0; pt < curve.size() - 1; ) {
        ...
        if (mid_dist_sqr > comp_dist * comp_dist) {
            pt += stepsize;
            stepsize *= 2;
        }
        else if (stepsize > 1)
...
bool Filter::isFree(Point const& fixed, Curve const& var_curve, PointID start,
    ...
    return comp_dist > 0 && mid_dist_sqr <= comp_dist * comp_dist;
}
...
void Filter::increase(std::size_t& step) {
    step = static_cast<std::size_t>(std::ceil(1.5 * step));
}

void Filter::decrease(std::size_t& step) {
    step /= 2;
}
...
bool Filter::bichromaticFarthestDistance() {
    ...
    d = Point{extreme1.min_x, extreme1.min_y}.dist_sqr(
        Point{extreme2.max_x, extreme2.max_y});
    if (d > distance_sqr)
        return false;
    d = Point{extreme1.min_x, extreme1.max_y}.dist_sqr(
        Point{extreme2.max_x, extreme2.min_y});
    if (d > distance_sqr)
        return false;
    d = Point{extreme1.max_x, extreme1.min_y}.dist_sqr(
        Point{extreme2.min_x, extreme2.max_y});
    if (d > distance_sqr)
        return false;
    d = Point{extreme1.max_x, extreme1.max_y}.dist_sqr(
...
bool Filter::adaptiveGreedy(PointID& pos1, PointID& pos2) {
    ...
    if (curve1[0].dist_sqr(curve2[0]) > distance_sqr
            || curve1.back().dist_sqr(curve2.back()) > distance_sqr)
        return false;
    ...
    while (pos1 + pos2 < curve1.size() + curve2.size() - 2) {
        ...
        if (curve1.size() - 1 == pos1) {
            auto new_pos2 = std::min<PointID::IDType>(pos2 + step,
                curve2.size() - 1);
        ...
        // if we have to do the step on curve 1
            ...
            auto new_pos1 = std::min<PointID::IDType>(pos1 + step,
                curve1.size() - 1);
        ...
        // if we cannot reduce the step size
            ...
            auto new_pos2 = std::min<PointID::IDType>(pos2 + step,
                curve2.size() - 1);
            bool step1_possible = isFree(curve2[pos2], curve1, pos1, new_pos1,
                distance);
            bool step2_possible = isFree(curve1[pos1], curve2, pos2, new_pos2,
                distance);
...
bool Filter::adaptiveSimultaneousGreedy() {
    ...
    while (pos1 + pos2 < curve1.size() + curve2.size() - 2) {
        ...
        // if we cannot reduce the step size
            ...
            PointID new_pos1 = std::min(pos1 + step, curve1.size() - 1);
            std::size_t step2 = step * (curve2.size() - pos2)
                / (curve1.size() - pos1);
            if (step2 < 1)
                step2 = 1;
            PointID new_pos2 = std::min(pos2 + step2, curve2.size() - 1);
...
bool Filter::negative(PointID position1, PointID position2) {
    ...
    std::size_t pos1 = position1;
    std::size_t pos2 = position2;
    for (std::size_t step = 1; pos1 + step <= curve1.size(); increase(step)) {
        auto cur_pos1 = pos1 + step - 1;
    ...
    for (std::size_t step = 1; pos2 + step <= curve2.size(); increase(step)) {
        auto cur_pos2 = pos2 + step - 1;
