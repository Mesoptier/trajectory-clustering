...
void FrechetLight::visAddReachable([[maybe_unused]] CInterval const& cinterval) {
#ifdef VIS
...
void FrechetLight::visAddUnknown([[maybe_unused]] CPoint begin,
        [[maybe_unused]] CPoint end, [[maybe_unused]] CPoint fixed_point,
        [[maybe_unused]] CurveID fixed_curve) {
#ifdef VIS
    if (begin >= end) { return; }
...
void FrechetLight::visAddConnection([[maybe_unused]] CPoint begin,
        [[maybe_unused]] CPoint end, [[maybe_unused]] CPoint fixed_point,
        [[maybe_unused]] CurveID fixed_curve) {
#ifdef VIS
    if (begin >= end) { return; }
...
void FrechetLight::visAddFreeNonReachable([[maybe_unused]] CPoint begin,
        [[maybe_unused]] CPoint end, [[maybe_unused]] CPoint fixed_point,
        [[maybe_unused]] CurveID fixed_curve) {
#ifdef VIS
    if (begin >= end) { return; }
...
inline CInterval FrechetLight::getInterval(Point const& point,
        Curve const& curve, PointID i) const {
    return getInterval(point, curve, i, nullptr);
}

inline CInterval FrechetLight::getInterval(Point const& point,
        Curve const& curve, PointID i, CInterval* outer) const {
    Interval outer_temp;
    Interval* outer_pt = outer == nullptr ? nullptr : &outer_temp;
    auto interval = IntersectionAlgorithm::intersection_interval(point,
        distance, curve[i], curve[i + 1], outer_pt);

    if (outer != nullptr) {
        *outer = CInterval{i, std::max(outer_temp.begin, 0.),
            i, std::min(outer_temp.end, 1.)};
    }
...
inline frechet::QSimpleInterval FrechetLight::getFreshQSimpleInterval(
        Point const& fixed_point, PointID min, PointID max,
        Curve const& curve) const {
    frechet::QSimpleInterval qsimple;
    updateQSimpleInterval(qsimple, fixed_point, min, max, curve);
    return qsimple;
}

inline bool FrechetLight::updateQSimpleInterval(
        frechet::QSimpleInterval& qsimple, Point const& fixed_point,
        PointID min, PointID max, Curve const& curve) const {
    assert((qsimple.getFreeInterval().is_empty()
            && qsimple.getOuterInterval().is_empty())
        || (!qsimple.getFreeInterval().is_empty()
            && !qsimple.getOuterInterval().is_empty()));
    if (qsimple.is_valid()
            || (qsimple.hasPartialInformation()
    ...
    if (qsimple.hasPartialInformation()) {
        if (qsimple.getLastValidPoint() < min) {
            qsimple = frechet::QSimpleInterval();
            //we know nothing about this part
        } else  {
            qsimple.clamp(CPoint{min, 0.}, CPoint{max, 0.}); 
            if (!qsimple.getFreeInterval().is_empty()) {
                return false;
                // even restricted to current interval, parent information still
                // gives invalidity 
            }
        }
    } 
    ...
    if (!qsimple.hasPartialInformation()) {
        // fresh computation, start with heuristics
        // heuristic check:
        auto mid = (min + max) / 2;
...
inline void FrechetLight::continueQSimpleSearch(
        frechet::QSimpleInterval& qsimple, Point const& fixed_point,
        PointID min, PointID max, Curve const& curve) const {
    ...
    // CERT: outerinterval -- note that it is safe to set last_empty = min even
    // if min is free, since we never add an empty interval of a single point
    // TODO: do we need to make sure that last_empty etc. is used only if we
    // want to certify or does the compiler optimization take care of it?
    CPoint last_empty = current_free ? CPoint{min, 0.} : CPoint{max + 1,0.};

    assert(first_free > max
        || fixed_point.dist_sqr(curve.interpolate_at(first_free)) <= dist_sqr);
    ...
    for (PointID cur = start; cur < max; ) {
        // heuristic steps:
        stepsize = std::min(stepsize, max - cur);
        ...
        if (current_free && comp_dist1 > 0
                && mid_dist_sqr <= comp_dist1 * comp_dist1) {
            cur += stepsize;
            stepsize *= 2;
            continue;
        }
        auto comp_dist2 = distance + maxdist;
        if (!current_free && mid_dist_sqr > comp_dist2 * comp_dist2) {
            cur += stepsize;
            stepsize *= 2;
            continue;
        }
        ...
        Interval outer;
        auto interval = IntersectionAlgorithm::intersection_interval(
            fixed_point, distance, cur_point, end_point, &outer);
        ...
        if (!current_free) {
            if (qsimple.is_valid()) {
                //we encountered a second free interval -> not qsimple
                qsimple.invalidate();
            ...
            if (interval.begin != 0 && interval.end != 1) {
                ...
                qsimple.setOuterInterval(CPoint{cur, outer.begin},
                                         CPoint{cur, outer.end});
                qsimple.validate();
            } 
            ...
            // end_dist_sqr <= dist_sqr does not hold, otherwise we would have
            // stopped before
    ...
    // ends at max.
    // Note: if no free points were found, first_free defaults to an invalid
    // value (>> max), making the interval empty   
    if (!qsimple.is_valid()) {
...
namespace {
    CIntervals::iterator getIntervalContainingNumber(
            CIntervals::iterator const& begin, CIntervals::iterator const& end,
            CPoint const& x) {
        auto it = std::upper_bound(begin, end, CInterval{x,
            CPoint{std::numeric_limits<PointID::IDType>::max(), 0.}});
        if (it != begin) {
            --it;
    ...
    CIntervals::iterator getIntervalContainingNumber(
            CIntervals::iterator const& begin, CIntervals::iterator const& end,
            PointID x) {
        auto it = std::upper_bound(begin, end, CInterval{x, 0.,
...
void FrechetLight::getReachableIntervals(frechet::BoxData& data) {
    ++num_boxes;
...
inline bool FrechetLight::emptyInputsRule(frechet::BoxData& data) {
    auto const& box = data.box;
...
inline void FrechetLight::boxShrinkingRule(frechet::BoxData& data) {
    auto& box = data.box;
...
inline void FrechetLight::handleCellCase(frechet::BoxData& data) {
    ...
    if (data.outputs.id1.valid()) {
        CInterval outer1;
        // TODO: set &outer1 to nullptr if we don't certify?
        // Probably not really costly...
        auto output1 = getInterval(curve2[box.max2], curve1, box.min1, &outer1);
        if (firstinterval2->is_empty()) {
            visAddFreeNonReachable(output1.begin,
                std::min(output1.end, firstinterval1->begin), {box.max2, 0.}, 1);
            output1.begin.setFraction(std::max(output1.begin.getFraction(),
    ...
    if (data.outputs.id2.valid()) {
        CInterval outer2;
        auto output2 = getInterval(curve1[box.max1], curve2, box.min2, &outer2);
        if (firstinterval1->is_empty()) {
            visAddFreeNonReachable(output2.begin,
                std::min(output2.end, firstinterval2->begin), {box.max1, 0.}, 0);
            output2.begin.setFraction(std::max(output2.begin.getFraction(),
...
inline void FrechetLight::getQSimpleIntervals(frechet::BoxData& data) {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
    auto const& box = data.box;
    ...
    if (data.outputs.id1.valid()) {
        if (data.qsimple_outputs.id1.valid())
            ...
            qsimple1 = frechet::QSimpleInterval();

        bool changed = updateQSimpleInterval(qsimple1, curve2[box.max2],
    ...
    if (data.outputs.id2.valid()) {
        if (data.qsimple_outputs.id2.valid())
            ...
            qsimple2 = frechet::QSimpleInterval();

        bool changed = updateQSimpleInterval(qsimple2, curve1[box.max1],
...
inline void FrechetLight::calculateQSimple1(frechet::BoxData& data) {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
    auto const& box = data.box;
        ...
        if (qsimple1.is_empty()) {
            ...
            if (x == box.min1 && pruning_level > 3 && enable_propagation1) {
                auto it = getIntervalContainingNumber(data.inputs.begin2,
                    data.inputs.end2, box.max2);
                if (it != data.inputs.end2) {
                    //(box.min1, box.max2) is reachable from interval *it 
                    CInterval &parent = *it; 
            ...
            // beginning of the free interval
                ...
                if (it != data.inputs.end1) {
                    //(x, box.min2) is reachable from interval *it 
                    auto interval = getFreshQSimpleInterval(
                        curve1.interpolate_at(x), box.min2, box.max2, curve2);
...
inline void FrechetLight::calculateQSimple2(frechet::BoxData& data) {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
    auto const& box = data.box;
    ...
    if (qsimple2.is_valid()) {
        ...
        if (qsimple2.is_empty()) {
            ...
            // beginning of the free interval
                ...
                if (it != data.inputs.end2) {
                    auto interval = getFreshQSimpleInterval(
                        curve2.interpolate_at(x), box.min1, box.max1, curve1);
                    if (interval.is_valid()) {
                        CInterval &parent = *it; 
                        out2 = qsimple2.getFreeInterval();
                        out2.begin = x;
...
inline bool FrechetLight::boundaryPruningRule(frechet::BoxData& data) {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
    auto const& box = data.box;

    // special cases for boxes which are at the boundary of the
    // free-space diagram
...
inline void FrechetLight::splitAndRecurse(frechet::BoxData& data) {
    auto const& box = data.box;
    ...
    if (box.max2 - box.min2 > box.max1 - box.min1) { // horizontal split
        ...
        frechet::BoxData data_bottom{
            ...
            {frechet::QSimpleID(), data.qsimple_outputs.id2}
        };
        ...
        frechet::BoxData data_left{
            ...
            {data.qsimple_outputs.id1, frechet::QSimpleID()}
        };
...
CPoint FrechetLight::getLastReachablePoint(Point const& point,
        Curve const& curve) const {
...
void FrechetLight::buildFreespaceDiagram(distance_t dist, Curve const& curve1,
        Curve const& curve2) {
    ...
    frechet::Box initial_box(0, curve1.size() - 1, 0, curve2.size() - 1);
    frechet::Inputs initial_inputs = computeInitialInputs();
    frechet::Outputs final_outputs = createFinalOutputs();
...
bool FrechetLight::lessThan(distance_t dist, Curve const& curve1,
        Curve const& curve2) {
    this->curve_pair[0] = &curve1;
    this->curve_pair[1] = &curve2;
    this->distance = dist;
    ...
    if (curve1.size() == 1 && curve2.size() == 1)
        return true;
    if (curve1.size() == 1)
        return isClose(curve1.front(), curve2);
    if (curve2.size() == 1)
        return isClose(curve2.front(), curve1);
...
bool FrechetLight::lessThanWithFilters(distance_t dist, Curve const& curve1,
        Curve const& curve2) {
    this->curve_pair[0] = &curve1;
    this->curve_pair[1] = &curve2;
    this->distance = dist;
...
inline void FrechetLight::computeOutputs(frechet::Box const& initial_box,
        frechet::Inputs const& initial_inputs,
        frechet::Outputs& final_outputs) {
    num_boxes = 0;

    frechet::BoxData box_data{initial_box, initial_inputs, final_outputs,
        frechet::QSimpleOutputs()};
    getReachableIntervals(box_data);
}

inline void FrechetLight::visAddCell([[maybe_unused]] frechet::Box const& box) {
#ifdef VIS
    cells.emplace_back(box.min1, box.min2);
#endif
}

inline bool FrechetLight::isClose(Point const& point,
        Curve const& curve) const {
    return getLastReachablePoint(point, curve)
        == CPoint{PointID(curve.size() - 1), 0.};
}

inline bool FrechetLight::isTopRightReachable(
        frechet::Outputs const& outputs) const {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
...
inline void FrechetLight::initCertificate(
        frechet::Inputs const& initial_inputs) {
    auto const& curve1 = *curve_pair[0];
    auto const& curve2 = *curve_pair[1];
...
inline auto FrechetLight::createFinalOutputs() -> frechet::Outputs {
    frechet::Outputs outputs;
...
inline auto FrechetLight::computeInitialInputs() -> frechet::Inputs {
    frechet::Inputs inputs;
...
distance_t FrechetLight::calcDistance(Curve const& curve1,
        Curve const& curve2) {
...
bool FrechetLight::isOnLowerRight(CPosition const& pt) const {
    return pt[0] == curve_pair[0]->size() - 1 || pt[1] == 0;
}
bool FrechetLight::isOnUpperLeft(CPosition const& pt) const {
...
Certificate& FrechetLight::computeCertificate() {
    ...
    if (curve1.back().dist_sqr(curve2.back()) > dist_sqr) { 
        cert.setAnswer(false);
        cert.addPoint({CPoint(curve1.size() - 1, 0.),
        ...
        while (cur_pos[0] > 0 or cur_pos[1] > 0) {
            ...
            next_pos[1 - interval->fixed_curve] =
                interval->end > cur_pos[1-interval->fixed_curve]
                    ? cur_pos[1 - interval->fixed_curve] : interval->end;
